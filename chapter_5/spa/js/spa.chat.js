...
spa.chat = (function () {
  ...
  //   * callback     ーアニメーションの最後のオプションのコールバック。
  ...
  onSetchatee = function ( event, arg_map ) {
    ...
    if ( ! new_chatee ) {
      if ( old_chatee )  {
        writeAlert( old_chatee.name + ' hs left the chat' );
      }
      else {
  ...
  //   * chat_modelーインスタントメッセージングとやり取りするメソッドを提供
  //     するチャットモデルオブジェクト。
  //   * people_modelーモデルが保持する人々のリストを管理するメソッドを
  //     提供するピープルモデルオブジェクト。
  //   * slider_*構成。すべてオプションのスカラー。完全なリストは
  //     configMap.settable_mapを参照。
